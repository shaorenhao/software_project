...
class ConceptAgent:
    def __init__(self):
        try:
            ...
            self.load_config()
            logger.info("ConceptAgent initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize ConceptAgent: {str(e)}")
            raise
    ...
    def load_config(self):
        ...
        try:
            ...
            if stats["doc_count"] == 0:
                logger.info("Initializing empty knowledge base")
    ...
    def _initialize_knowledge_base(self):
        """Initialize knowledge base with enhanced data"""
        try:
            # 加载基础概念数据
            basic_concepts = self._get_basic_concepts()
            self.vector_db.batch_insert(basic_concepts)
            logger.info(f"Initialized knowledge base with {len(basic_concepts)} concepts")
        except Exception as e:
            logger.error(f"Failed to initialize knowledge base: {str(e)}")
            raise

    def _get_basic_concepts(self) -> List[Dict]:
        """Return enhanced basic concepts data"""
        return [
        # 面向对象编程概念
        {
    ...
    def _initialize_knowledge_graph(self):
        """Initialize knowledge graph with enhanced relationships"""
        try:
            # 核心编程范式
            oop = self.knowledge_graph.add_concept("面向对象编程", "编程范式", {
                "definition": "使用对象和类组织代码的编程范式",
                "importance": "high"
            })
            
            # 更丰富的概念和关系
            self._add_oop_concepts()
    ...
    def _add_oop_concepts(self):
        """Add OOP concepts with relationships"""
        concepts = [
            ("多态", "OOP特性", "同一操作作用于不同对象可以有不同的解释"),
            ("封装", "OOP特性", "隐藏对象内部实现细节"),
            ("继承", "OOP特性", "子类继承父类的属性和方法"),
            ("抽象", "OOP特性", "提取共同特征而忽略非本质细节")
        ]
        
        for name, typ, definition in concepts:
            self.knowledge_graph.add_concept(name, typ, {"definition": definition})
            self.knowledge_graph.add_relation(name, "属于", "面向对象编程")
            self.knowledge_graph.add_relation(name, "实现", "SOLID原则")

    def _add_design_patterns(self):
        """Add design patterns with relationships"""
        patterns = [
            ("工厂模式", "设计模式", "创建对象的接口，让子类决定实例化哪个类"),
            ("观察者模式", "设计模式", "对象间的一对多依赖关系"),
            ("策略模式", "设计模式", "定义一系列算法并使它们可以互相替换")
        ]
        
        for name, typ, definition in patterns:
            self.knowledge_graph.add_concept(name, typ, {"definition": definition})
            self.knowledge_graph.add_relation(name, "应用", "多态")
            self.knowledge_graph.add_relation(name, "遵循", "SOLID原则")

    @lru_cache(maxsize=100)
    def get_knowledge_context(self, concept_name: str) -> str:
        """Get context from knowledge graph with caching"""
        try:
            related = self.knowledge_graph.query_related_concepts(concept_name, depth=2)
            if not related:
                return ""
            
            context = "\n🌐 知识图谱关联:\n"
            for item in related:
                concept_name = item["concept"].get("name", "未知概念")
                relation_names = []
                for rel in item["relations"]:
                    rel_type = rel.get("type", "关系")
                    relation_names.append(f"{rel_type}")
                path = " → ".join(relation_names)
                context += f"- {concept_name} ({path})\n"
    ...
    def chat(self, messages: List[Dict], use_rag: bool = False, use_kg: bool = False) -> Dict:
        """Generate knowledge-enhanced response with improved retrieval"""
        if not messages:
        ...
        try:
            # 知识图谱检索
            if use_kg:
                kg_results = self.knowledge_graph.semantic_search(last_msg)
                if kg_results:
                    context += self.get_knowledge_context(kg_results[0]["name"])
            
            # 向量检索
            if use_rag:
                vector_results = self.vector_db.search(last_msg, top_k=3)
                if vector_results:
                    context += "\n📚 相关概念:\n" + "\n".join(
                        f"{i+1}. {res['content']}" 
